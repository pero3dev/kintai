...
## 3. 可用性（障害時リカバリ）
- [x] DB障害時の復旧試験（停止→復帰、再接続）
- [x] Redis障害時の復旧試験（停止→復帰、再接続）
- [x] アプリ再起動時の継続性試験（in-flight request含む）
- [x] フェイルオーバー試験（複数インスタンス想定）
- [x] readiness/liveness劣化時の挙動試験（依存先断）
- [x] バックアップ/リストア訓練（RPO/RTO検証）
- [x] ネットワーク分断/タイムアウト時の回復試験

## 4. 互換性（ブラウザ/端末差）
- [x] FirefoxでのE2E実行
- [x] WebKit/SafariでのE2E実行
- [x] Edge互換実行
- [x] モバイル端末エミュレーションE2E（iOS/Android）
- [x] タブレット/画面幅別のE2E検証
- [x] CIでのブラウザ×端末マトリクス実行

## 推奨実装順序（P0 -> P3）
- [x] P0: 互換性基盤（Playwright複数ブラウザ化）+ 性能SLO測定基盤
- [x] P1: APIレスポンス時間/スループット + 高並列負荷
- [x] P2: 長時間耐久 + 可用性（DB/Redis障害復旧）
- [x] P3: フェイルオーバー + バックアップ/リストア演習
