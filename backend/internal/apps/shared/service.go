...
// NewServices は共有サービスを初期化する
func NewServices(deps Deps, attendanceRepo AttendanceQueryRepo, leaveRepo LeaveQueryRepo, overtimeRepo OvertimeQueryRepo) *Services {
	...
	return &Services{
		...
		Dashboard:    NewDashboardService(deps, attendanceRepo, leaveRepo),
		...
		Export:       NewExportService(deps, attendanceRepo, leaveRepo, overtimeRepo),
	}
}
...
type dashboardService struct {
	deps           Deps
	attendanceRepo AttendanceQueryRepo
	leaveRepo      LeaveQueryRepo
}

func NewDashboardService(deps Deps, attendanceRepo AttendanceQueryRepo, leaveRepo LeaveQueryRepo) DashboardService {
	return &dashboardService{deps: deps, attendanceRepo: attendanceRepo, leaveRepo: leaveRepo}
}

func (s *dashboardService) GetStats(ctx context.Context) (*model.DashboardStatsExtended, error) {
	// 今日の出勤者数
	todayPresent, _ := s.attendanceRepo.CountTodayPresent(ctx)
	...
	// 承認待ちの休暇申請数
	pendingLeaves, _ := s.leaveRepo.CountPending(ctx)

	// 今月の残業時間
	now := time.Now()
	monthStart := time.Date(now.Year(), now.Month(), 1, 0, 0, 0, 0, time.Local)
	monthEnd := monthStart.AddDate(0, 1, 0).Add(-time.Second)
	monthlyOvertime, _ := s.attendanceRepo.GetMonthlyOvertime(ctx, monthStart, monthEnd)
	...
	for i := 6; i >= 0; i-- {
		...
		records, _ := s.attendanceRepo.FindByDateRange(ctx, dayStart, dayEnd)
...
type exportService struct {
	...
	attendanceRepo AttendanceQueryRepo
	leaveRepo      LeaveQueryRepo
	overtimeRepo   OvertimeQueryRepo
}

func NewExportService(deps Deps, attendanceRepo AttendanceQueryRepo, leaveRepo LeaveQueryRepo, overtimeRepo OvertimeQueryRepo) ExportService {
	return &exportService{deps: deps, attendanceRepo: attendanceRepo, leaveRepo: leaveRepo, overtimeRepo: overtimeRepo}
}
...
func (s *exportService) ExportAttendanceCSV(ctx context.Context, userID *uuid.UUID, start, end time.Time) ([]byte, error) {
	...
	if userID != nil {
		attendances, _, _ := s.attendanceRepo.FindByUserAndDateRange(ctx, *userID, start, end, 1, 10000)
		...
		attendances, _ := s.attendanceRepo.FindByDateRange(ctx, start, end)
		for _, a := range attendances {
			userName := ""
			if a.User != nil {
...
func (s *exportService) ExportLeavesCSV(ctx context.Context, userID *uuid.UUID, start, end time.Time) ([]byte, error) {
	...
	for _, u := range users {
		...
		leaves, _, _ := s.leaveRepo.FindByUserID(ctx, u.ID, 1, 10000)
...
func (s *exportService) ExportOvertimeCSV(ctx context.Context, start, end time.Time) ([]byte, error) {
	...
	now := time.Now()
	for _, u := range users {
		monthly, _ := s.overtimeRepo.GetUserMonthlyOvertime(ctx, u.ID, now.Year(), int(now.Month()))
		yearly, _ := s.overtimeRepo.GetUserYearlyOvertime(ctx, u.ID, now.Year())
