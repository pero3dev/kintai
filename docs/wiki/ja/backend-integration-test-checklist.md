...
## 5. 共通ドメイン（shared）
- [x] 通知（一覧/未読件数/既読化/全既読/作成）
- [x] ユーザー（CRUD/ロール更新/`/users/me`）
- [x] 部署管理（CRUD）
- [x] シフト（週次作成/一括作成/個別更新/作成）
- [x] プロジェクト（CRUD/フィルタ）
- [x] 休日管理（CRUD/重複検知/プロジェクト横断検索）
- [x] 祝日管理（CRUD/年度検索/カレンダー/CSV入出力）
- [x] 修正申請フロー（CRUD/ステータス遷移）
- [x] エクスポート（attendance/leaves/overtime/projects）
- [x] ダッシュボード集計

## 6. 経費ドメイン（expense）
- [x] 経費申請作成（項目、税区分、カテゴリ計算）
- [x] 経費申請更新（ステータス制約）と削除
- [x] 承認・却下（権限、ステータス遷移、通知）
- [x] 一覧/検索/レポート/月次集計
- [x] CSV/PDFエクスポート
- [x] 領収書アップロード（multipart）
- [x] コメント追加/取得/削除、履歴取得
- [x] テンプレートCRUD/テンプレート適用
- [x] ポリシーCRUD/超過検知アラート
- [x] 通知設定/リマインダー/通知履歴
...
## 9. 回帰重点
- [x] 日付境界（UTC/JST、月跨ぎ、締め日）での4xx/正常系
- [x] 複数ロール混在時のデータ漏洩防止
- [x] 打刻修正・経費承認の同時更新で整合性維持

## 10. 推奨実装順序（P0 -> P3）
- [x] P0: テスト基盤 + 認証/ミドルウェア + 全endpoint共通軸
- [x] P1: attendance + shared（主要業務）
- [x] P2: expense + HR（中長期機能）
- [x] P3: DB制約・同時更新・高負荷寄り検証

## 完了条件
- [x] 全199 endpointが共通軸テストで検証済み
- [x] 各ドメインの主要シナリオが最低1件以上実装済み
- [x] 主要永続化処理に実DB結合テストが存在
- [x] CI実行で再現可能（ローカル/CIとも同等）
